...
TARGET_BY_ID = {
    ...
    "last-stand": "自己的当前4名队员和全体可用非队长选手",
    "open-feast": "自己",
    ...
    "stuck-together": "同阵营、未被选走的可选选手",
    "storm-fog": "起点队长，随后按顺位环形影响最多3名非使用者队长",
    "snow-cat": "非自己的任意队长",
    "charged-cannon": "自己或一名未行动队长",
    "heavenly-descent": "刚被其他队长购买的同阵营选手",
}

TIPS_BY_ID = {
    "snow-cat": ["适合干扰正在找关键卡的其他队长，不能对自己使用。", "目标仍按显示费用付款，购买后才揭示真实选手。"],
    "heavenly-descent": ["适合反制对手刚买到的同阵营高费关键卡，跨阵营目标不能发动。", "发动者最好预留队伍空位。"],
    ...
    "stuck-together": ["适合提前锁定同阵营关键选手，跨阵营目标不能选择。", "目标被其他规则拿走时会失效。"],
    ...
    "storm-fog": ["用于连续影响多个后续行动队长，适合关键轮次压节奏。", "本轮不足3名合法目标时会顺延到下一轮，刷新商店不会清除血雾。"],
}
...
SPECIAL_JUDGE_NOTES = {
    ...
    "hungry-wave": ["触发者金币清零并跳过本轮。", "命中同阵营则夺取，命中异阵营则退回购买并轮末补偿。"],
    "last-stand": ["会放弃当前4名队员并重构阵容。", "被抽走队员的队伍需要按规则获得置换补偿。"],
    "heavenly-descent": ["必须在购买后10秒响应窗口内处理。", "只能夺取发动者同阵营选手，跨阵营购买不可发动。", "原购买队长返还金币和购买权，不返还刷新次数。"],
    "decompose-knowledge": ["每个选人阶段最多叠到3层。", "发动后消耗全部3层。"],
    "stuck-together": ["目标必须同阵营、未被选走且不能是队长。", "指定目标后延迟到下一轮选人开始判定。", "目标若已离开卡池则失效。"],
    "storm-fog": ["按顺位环形向后影响最多3名仍有购买权且未满员的非使用者队长。", "本轮不足3名合法目标时顺延到下一轮。", "刷新商店不会清除血雾，购买权结束后清除。"],
    "snow-cat": ["不能对自己使用，只能选择其他未满员队长。", "目标下一次商店信息打乱。", "购买后才揭示真实选手，费用按显示卡牌结算。"],
    "charged-cannon": ["转换技二选一。", "每轮最多使用1次。"],
