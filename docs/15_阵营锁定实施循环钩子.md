...
该脚本是任务文档驱动的执行循环入口，固定读取 `docs/14_阵营锁定10队金币商店模式_实施规范.md`。它会自动提取目标、禁止项、验收标准、推荐执行顺序、当前状态和阻断线索，并自动执行 `npm test` 与 `git diff --check`。

该脚本不能替代模型级执行、Build Web Apps 审查或 Codex Security 审查；但当 `--status=complete` 时，如果任务文档仍存在未勾选清单、环境阻断、待处理、尚未完成、等待用户、失败、TODO 等线索，脚本必须失败退出。
...
- 不得跳过 `npm test` 和 `git diff --check`，除非只是诊断钩子本身且显式使用 `--skip-gates=true`。
- 不得提交无关文件或临时产物。
